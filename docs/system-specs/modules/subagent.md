...
- `cwd` (str, optional): absolute path to launch subagent in. Must be under a configured `subagent_cwd_allowed_roots` entry (default: `~/workspace`, `~/workspaces`, `~/workplace`, `~/workplaces`). Validated via realpath + prefix match. Pool skipped when cwd is set. These roots are a least-privilege allowlist: a persisted list whose roots all fail to exist on the host rejects every cwd, and the operator must edit `agent.subagent_cwd_allowed_roots` (or delete the key to take the shipped default). Neither the loader nor the guard stats the configured roots.

  **One root is added implicitly: the parent session's own project directory.** `admission/gate.py` reads it off the parent's live provider `cwd` (`_parent_project_root`, realpath'd) and appends it for that spawn only. The rationale is that it is not a new grant — the user chose that project for this session, `include_project` already ships its context to the child, and the child's tools run the same code the parent's do. Without it, every spawn in a project outside the four shipped work-tree roots was refused and retried with no cwd, which silently moved the child to an unrelated directory and made its file reads answer for the wrong tree. Scope limits, and they are the invariant here: it applies **only** when `allowed_roots` is non-empty, so `subagent_cwd_allowed_roots=[]` keeps meaning "no cwd overrides at all" and is never re-enabled by this path; it adds exactly one directory, never its parents; and a parent with no live provider (a cron, a headless caller, a disposed session) contributes nothing, leaving the configured list to decide alone. `_parent_project_root` never raises, so a failure to read the parent narrows rather than widens.
